...
// Optional Windows path to an AUDIO backing track. The video backdrop is muted
// by design, so this separate file is the audible accompaniment in playback.
const BACKING = process.env.BACKING;
const BUNDLE_ROOT = process.env.BUNDLE_ROOT || "/mnt/e/projects/RockCraft";
const URL = `ws://${ADDR}`;
// Demo pacing: DELAY_MS pauses between beats so a human can watch the host app
// react (0 = full-speed automated run). HOLD_MS keeps the app open before the
// final app_quit so the end state is visible.
...
const pause = (ms = DELAY_MS) => (ms > 0 ? new Promise((r) => setTimeout(r, ms)) : Promise.resolve());
...
try {
  ...
  // Beat 1b (optional) — attach an audible audio backing track alongside the
  // (muted) video backdrop. Persisted into the bundle on save and re-attached by
  // play_load, so it plays during the playback window.
  if (BACKING) {
    await host(15, "attach_backing", { path: BACKING });
    console.log(`• attach_backing → ${BACKING} 🔊`);
  }
  await pause();

  // Beat 2 — author 4 ascending notes (one at a time so each is visible).
  const melody = [[60, 0], [62, 4], [64, 8], [65, 12]];
  let i = 0;
  for (const [pitch, step] of melody) {
    await action(100 + i, "set_cursor", { pitch, step });
    await pause(Math.floor(DELAY_MS / 2));
    await action(200 + i, "add_note", {});
    console.log(`  · note ${i + 1}/4 → pitch ${pitch} @ step ${step}`);
    i++;
    await pause();
  }
  ...
  // Beat 2b (demo + backing only) — play the timeline back in the EDIT view so
  // the attached audio backing is audible (M5 "audible backing while editing"):
  // the transport drives sync_backing → the backing track plays, and the
  // playhead crossing each note auditions it through the synth. Then stop.
  if (BACKING && DELAY_MS > 0) {
    console.log("• play_from_start → backing track + note auditions in edit 🔊");
    await action(50, "play_from_start", {});
    await pause(6000);
    await action(51, "stop", {});
    await pause();
  }
  ...
  // Hold the end state on screen before quitting, when demoing.
  if (HOLD_MS > 0) {
    console.log(`• holding ${HOLD_MS}ms before quit …`);
    await pause(HOLD_MS);
  }
