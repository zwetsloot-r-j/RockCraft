// song.ts — "Ember Lantern" mock fixture, ported from the design prototype's
// song.js. A short original loop in A-minor, 120 BPM. Left hand = sustained
// bass (root + fifth); right hand = flowing 8th-note arpeggio + a held
// top-line melody. vi–IV–I–V (Am–F–C–G), 4 bars, seamless.
//
// This is the mock data source; #168 (play-screen live wiring) replaces it
// with a real bundle converted from core::NoteSpan at the boundary.
...
for (let b = 0; b < BARS; b++) {
  ...
  notes.push({ note: ch.bass[0], start: bar0, end: bar0 + BEAT * 2, hand: "L" });
  notes.push({ note: ch.bass[1], start: bar0 + BEAT * 2, end: bar0 + BEAT * 4, hand: "L" });
  // RH — 8 eighth-note arpeggio
  for (let i = 0; i < 8; i++) {
    const t = bar0 + i * (BEAT / 2);
    notes.push({ note: ch.rh[ARP[i]], start: t, end: t + BEAT / 2 - 45, hand: "R" });
  }
  // RH — held top-line melody (octave above the chord's high tone)
  notes.push({ note: ch.rh[2] + 12, start: bar0, end: bar0 + BEAT * 1.5, hand: "R" });
}
